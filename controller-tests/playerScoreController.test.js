...
test('Should not be able to create when username is longer than 16 character', done => {
    request(app).post('/').type('json').send({name: 'Asuperlongnamecontaining16characters', score: 90204}).expect(400, {message: "Invalid name created. try again"}, done)
})


test('Should not be able to submit an empty string as a name', done => {
    request(app).post('/').type('json').send({name: '', score: 1000000}).expect(400, {message: "Invalid name created. try again"}, done)
})

test('There should be an type error when score is not correctly provided ', done => {
    request(app).post('/').type('json').send({name: "James", score: ""}).expect(400, {error: "Error creating score"}, done)
})

test('There should be an type error when name is not correctly provided ', done => {
    request(app).post('/').type('json').send({name: 1000, score: 50000}).expect(400, {error: "Error creating score"}, done)
})
